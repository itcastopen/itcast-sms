...
public class SmsSendServiceImpl implements SmsSendService {
     ...
     * @param params
     */
    private void checkSensitive(Collection<String> params) {
        String sensitive = sensitiveListService.listByType("1"); // 短信
        for (String param : params) {
            if (sensitive.contains(param + ",")) {
                throw new SmsException("敏感词参数:" + param);
            }
        }
    }
    ...
    private void sendSmsMessage(SmsSendDTO smsSendDTO, PlatformEntity platformEntity) {
        ...
        // 4、校验短信内容是否含有敏感词
        checkSensitive(smsSendDTO.getParams().values());
