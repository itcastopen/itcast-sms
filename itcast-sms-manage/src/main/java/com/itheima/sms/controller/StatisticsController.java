...
public class StatisticsController extends BaseController {
    ...
    public R<Page<MarketingStatisticsCountVO>> marketingCountPage(MarketingDTO marketingDTO) {
        ...
        if (getEndCreateTime() != null)
            params.put("endCreateTime", DateUtils.format(getEndCreateTime(), DateUtils.DEFAULT_DATE_TIME_FORMAT));
        params.put("signatureName", marketingDTO.getSignatureName());
        params.put("title", marketingDTO.getTitle());
        params.put("status", 1);
        IPage<MarketingDTO> marketingPage = marketingService.customPage(getPage(), params);

        List<MarketingStatisticsCountVO> records = marketingPage.getRecords().stream().map(item -> {
            MarketingStatisticsCountVO marketingStatisticsCountVO = sendLogService.getMarketingCountByBusinessId(item.getId());
            marketingStatisticsCountVO.setId(item.getId());
            marketingStatisticsCountVO.setTitle(item.getTitle());
            marketingStatisticsCountVO.setSignatureName(item.getSignatureName());
            marketingStatisticsCountVO.setDate(DateUtils.format(item.getCreateTime(), "yyyy-MM-dd HH:mm:ss"));
            return marketingStatisticsCountVO;
        }).collect(Collectors.toList());
        Page<MarketingStatisticsCountVO> page = new Page<>(marketingPage.getCurrent(), marketingPage.getSize(), marketingPage.getTotal());
